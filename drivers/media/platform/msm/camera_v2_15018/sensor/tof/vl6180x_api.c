...
int VL6180x_InitData(VL6180xDev_t dev){
    ...
    int8_t offset;
    uint8_t FreshOutReset;
	uint8_t model_major, model_minor;
	uint8_t module_major, module_minor;
    ...
    while(0);
		...
		// Read Model Version
	status = VL6180x_RdByte(dev, 0x0001,(uint8_t*)&model_major);
	status = VL6180x_RdByte(dev, 0x0002, (uint8_t*)&model_minor);
	pr_err("STM VL6180 Model Version : %d.%d\n", model_major,model_minor);

	// Read Module Version
	status = VL6180x_RdByte(dev, 0x0003,(uint8_t*)&module_major);
	status = VL6180x_RdByte(dev, 0x0004,(uint8_t*)&module_minor);
	pr_err("STM VL6180 Module Version : %d.%d\n",module_major,module_minor);
