...
static int  gtp_get_sensor_id(int TP_ID1, int TP_ID2, int TP_ID3, struct goodix_ts_platform_data *pdata)
{
    ...
    if (is_project(OPPO_15025))
    {
        ...
        {
            case TP_TPK:
                strcat(gt9xx_updata_file_path, "tp/15025/15025_GOODIX_UPDATE_TPK.bin");
                break;
            case TP_OFILM:
            case TP_TRULY:
            default:
    ...
    if (is_project(OPPO_15029))
    {
        ...
        {
            case TP_TPK:
                strcat(gt9xx_updata_file_path, "tp/15029/15029_GOODIX_UPDATE_TRULY.bin");
                break;
            case TP_OFILM:
            case TP_TRULY:
            default:
...
//      goto exit_deinit_power;
    ...
    {
        pr_err("gtp regulator_disable is called,boot_mode = %d\n", boot_mode);
        if (gpio_is_valid(ts->pdata->reset_gpio))
        {
            pr_err("gtp:enable the reset_gpio\n");
            gpio_direction_output(ts->pdata->reset_gpio, 0);
            msleep(5);
        }
        if (regulator_count_voltages(ts->vdd) > 0)
        {
            ...
            if (ret)
            {
                pr_err("gtp vdd regulator_count_voltages failed,errcode = %d\n", ret);
            }
        }
        return ret;
    }
