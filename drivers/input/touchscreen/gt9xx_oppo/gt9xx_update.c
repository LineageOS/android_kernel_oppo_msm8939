...
extern char gt9xx_updata_file_path[128];
#define UPDATE_FILE_PATH_1           gt9xx_updata_file_path //"/data/_goodix_update_.bin"
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
            ...
            (!memcmp(fw_head->pid, "91XX", 4)))
    {
        ...
        //The third condition
        if (fw_head->vid != update_msg.ic_fw_msg.vid || gt9xx_force_update == 1)
        {
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* path)
{
    ...
    if (path)
    {
        GTP_ERROR("Update File path:%s, %zd", path, strlen(path));
...
s32 gup_update_proc(void *dir)
{
    ...
    printk(KERN_EMERG"[update_proc]Begin update ......");
    ...
    GTP_DEBUG("[update_proc]retry ok ?.");
