...
static void gesture_judge(struct synaptics_ts_data *ts)
{
	...
	switch (gesture_sign) {
		...
		case VEE_DETECT:
			...
			else if (gesture == UpVee && ts->letter_v_enable)
			...
			gesture = (gesture_buffer[2] == 0x77 &&
				  ...
				  (gesture_buffer[2] == 0x6d &&
				   gesture_buffer[3] == 0x00) ? Mgesture :
								UnkownGesture;
			if (gesture == Wgesture && ts->letter_w_enable)
				keycode = KEY_GESTURE_W;
			else if (gesture == Mgesture && ts->letter_m_enable)
...
static int init_synaptics_proc(struct synaptics_ts_data *ts)
{
	...
	if (ts->black_gesture_support) {
		...
		prEntry_tmp = proc_create("letter_v_enable", 0600,
				prEntry_tp, &tp_letter_v_proc_fops);
		if (prEntry_tmp == NULL) {
			ret = -ENOMEM;
			pr_err("%s: Couldn't create proc entry\n", __func__);
		}
