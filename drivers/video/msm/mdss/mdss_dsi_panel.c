...
static int lcd_dev = LCD_UNKNOWN;
...
/* YongPeng.Yi@SWDP.MultiMedia, 2015/05/29  Add for 15009 cabc START */
		if((is_project(OPPO_15109)) && ctrl->ndx==0){
			set_backlight_pwm(1);
...
/* Xiaori.Yuan@Mobile Phone Software Dept.Driver, 2015/04/10  Add for esd check */
	if(MSM_BOOT_MODE__NORMAL!=get_boot_mode() && (is_project(OPPO_14005) || is_project(OPPO_15011) || is_project(OPPO_15018) || is_project(OPPO_15022))){
		pinfo->esd_check_enabled = false;
	}
#endif /*CONFIG_MACH_OPPO*/
...
/* YongPeng.Yi@SWDP.MultiMedia, 2015/06/11  Add for lcd type START */
	if(!strcmp(panel_name,"oppo15109boe ili9881c 720p video mode dsi panel")){ /*lile@EXP.BasicDrv.LCD, 2016-01-29, add for 15309 Boe LCD*/
