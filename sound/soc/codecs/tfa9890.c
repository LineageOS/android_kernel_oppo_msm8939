...
/* presets tables for different modes */
...
static char const *tfa9890_eq_tables[] = {
	"tfa9890_music_table.eq",
	"tfa9890_voice_table.eq",
};
...
static const struct firmware *fw_eq_table[ARRAY_SIZE(tfa9890_mode)];
			...
			int len, int type, u8 *read)
{
	...
	err = codec->bulk_write_raw(codec, TFA9890_CF_CONTROLS, buffer,
						ARRAY_SIZE(buffer));
	if (err < 0) {
		pr_err("tfa9890: Failed to Write DSP controls req: %d\n", err);
		return err;
	}
	/* check transfer type */
	if (type == TFA9890_DSP_WRITE) {
		...
		if (err < 0) {
			pr_err("tfa9890: Memory upload failed\n");
			return err;
		}
	}
	...
	if ((cf_status & TFA9890_STATUS_CF) == 0) {
		pr_err("tfa9890: Failed to ack DSP CTL req: %d\n", err);
		return -EIO;
	}
	/* read the RPC Status */
	...
	err = codec->bulk_write_raw(codec, TFA9890_CF_CONTROLS, buffer, 4);
	if (err < 0) {
		pr_err("tfa9890: Failed to Write NXP DSP CTL reg for "
			"rpc check: %d\n", err);
		return err;
	}
...
static int tfa9890_set_mode(struct tfa9890_priv *tfa9890)
{
	...
	if ((fw_pst_table[tfa9890->mode])->size != TFA9890_PST_FW_SIZE) {
		pr_err("tfa9890: Data size check failed for preset file\n");
		return -EIO;
	}

	pr_debug("tfa9890: switching to mode: %s\n",
	...
	ret = tfa9890_dsp_transfer(tfa9890->codec, TFA9890_DSP_MOD_SPEAKERBOOST,
			...
			fw_pst_table[tfa9890->mode]->size,
			TFA9890_DSP_WRITE, 0);
	if (ret < 0)
		return ret;

	ret = tfa9890_dsp_transfer(tfa9890->codec, TFA9890_DSP_MOD_BIQUADFILTERBANK,
			0, fw_eq_table[tfa9890->mode]->data,
			fw_eq_table[tfa9890->mode]->size,
			TFA9890_DSP_WRITE, 0);
	if (ret < 0)
		return ret;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (mode_value > (ARRAY_SIZE(tfa9890_mode) - 1) || mode_value < 0) {
		pr_err("%s: invalid mode set\n", __func__);
...
{
	u8 coldpatch[7] = {0x00, 0x07, 0x81, 0x00, 0x00, 0x00, 0x01};
	int ret;
	u16 val;
...
static int tfa9890_load_config(struct tfa9890_priv *tfa9890)
{
	...
	if (fw_speaker->size != TFA9890_SPK_FW_SIZE) {
		pr_err("tfa9890: Data size check failed for speaker model\n");
		goto out;
	}
	...
	if (ret < 0) {
		pr_err("tfa9890: Speaker model upload failed\n");
		goto out;
	}
	...
	if (fw_config->size != TFA9890_CFG_FW_SIZE) {
		pr_err("tfa9890: Data size check failed for config file\n");
		goto out;
	}
	ret = tfa9890_dsp_transfer(codec, TFA9890_DSP_MOD_SPEAKERBOOST,
	...
	if (ret < 0) {
		pr_err("tfa9890: Config upload failed\n");
		goto out;
	}
...
static void tfa9890_load_preset(struct work_struct *work)
{
	...
	file_name = kzalloc(FIRMWARE_NAME_SIZE, GFP_KERNEL);
	if (!file_name) {
	...
	for (i = 0; i < ARRAY_SIZE(fw_pst_table); i++) {
		scnprintf(file_name, FIRMWARE_NAME_SIZE, "%s/%s.%s",
				tfa9890->fw_path, tfa9890->tfa_dev,
				tfa9890_preset_tables[i]);
		ret = request_firmware(&fw_pst_table[i],
				file_name,
				codec->dev);
		if (ret || fw_pst_table[i]->size != TFA9890_PST_FW_SIZE) {
			pr_err("%s: Failed to locate DSP preset table %s\n",
					codec->name, file_name);
			tfa9890->dsp_init = TFA9890_DSP_INIT_FAIL;
			break;
		}
		...
		scnprintf(file_name, FIRMWARE_NAME_SIZE, "%s/%s.%s",
				tfa9890->fw_path, tfa9890->tfa_dev,
				tfa9890_eq_tables[i]);
		ret = request_firmware(&fw_eq_table[i],
				file_name, codec->dev);
		if (ret || fw_eq_table[i]->size != TFA9890_COEFF_FW_SIZE) {
			pr_err("%s: Failed to locate EQ table %s\n",
					codec->name, file_name);
			tfa9890->dsp_init = TFA9890_DSP_INIT_FAIL;
			break;
		}
...
static void tfa9890_dsp_init(struct work_struct *work)
{
	...
	pr_info("tfa9890: Initializing DSP, status:0x%x\n", val);
			     ...
			     struct snd_soc_dai *dai)
{
	...
	if (params_format(params) != SNDRV_PCM_FORMAT_S16_LE) {
		pr_err("tfa9890: invalid pcm bit length\n");
		return -EINVAL;
	}
